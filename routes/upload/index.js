  ...
  files.forEach(item => {
    ...
    fs.rename(oldpath,newPath,()=>{
      sizeOf(newPath, function (err, dimensions) {
        ...
        let sql
        if(id === 'null'){
          sql = `insert into upload values (${admin.id},${time},'${uploadAddress}','${proportion}','${defaultDescription}',0,0)`
        }else{
          sql = `insert into upload values (${id},${time},'${uploadAddress}','${proportion}','${defaultDescription}',0,0)`
        }
